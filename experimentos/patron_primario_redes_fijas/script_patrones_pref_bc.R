...
patron_pref_bc <- read.csv("redes_preferenciales_data/main-pattern-bc-fixed-pref-table.csv", skip = 6)
...
patrones_por_configuracion <- patron_pref_bc %>% 
...
rmse_por_corrida <- patron_pref_bc %>%
  filter(tick %in% c(69, 166, 259)) %>%
...
simulation_best_config_hombres <- read.csv("redes_preferenciales_data/modelo_redes_preferenciales best-config-bc-hombres-pref-table",
                                           skip= 6) 
...
simulation_best_config_hombres_plot <- ggplot(simulation_best_config_hombres_largo, aes(x = tick, group = grupo)) +
  ...
  labs(
    title = "Intención de voto bajo confianza acotada en redes preferenciales,
...
ggsave("figures_pref/bc_patron_hombres_pref.pdf", 
       plot = simulation_best_config_hombres_plot,
       width = 16, height = 10, units = "cm")

#### Simulación de mejor configuración general ####
simulation_best_config_general <- read.csv("redes_preferenciales_data/modelo_redes_preferenciales best-config-bc-general-pref-table",
                                           skip= 6) 
...
# Exportación con las medidas calculadas para los márgenes del documento
ggsave("figures_pref/bc_patron_general_pref.pdf", 
...
simulation_best_config_mujeres <- read.csv("redes_preferenciales_data/modelo_redes_preferenciales best-config-bc-mujeres-pref-table",
                                           skip= 6) 
...
# Exportación con las medidas calculadas para los márgenes del documento
ggsave("figures_pref/bc_patron_mujeres_pref.pdf", 
