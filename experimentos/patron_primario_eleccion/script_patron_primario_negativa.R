#### Este script procesa los datos obtenidos del experimento main-pattern-negative
...
redes_patron_principal_negativa <-  read.csv("behavior_space_datos/modelo_redes_preferenciales main-pattern-negative-table.csv", skip = 6)
...
redes_patron_principal_procesada <- redes_patron_principal_negativa %>% 
  rename(
    tick = X.step., 
...
redes_patron_principal_filtrada <- redes_patron_principal_procesada %>% 
  ...
  filter(confidence_threshold <= 1.6) # Este es el valor critico despues del cual el comportamiento es igual a influencia positiva


#### Calculo del RMSE ####

# Se calcula el RMSE por cada una de las ejecuciones
redes_rmse_ejecucion_negativa <- redes_patron_principal_filtrada %>% 
...
# 30 repeticiones
redes_rmse_stats_negativa <- redes_rmse_ejecucion_negativa %>% 
...
redes_best_configs_negativa <- redes_rmse_stats_negativa %>% 
...
redes_sensitivity_plot_rmse_negativa <- ggplot(redes_rmse_stats_negativa, aes(x = pairs_per_tick, y = learning_rate, 
                                                                  fill = mean_RMSE)) + 
  geom_tile() + 
  ...
  labs(
    title = "RMSE across 3 parameters for negative influence 
...
print(redes_sensitivity_plot_rmse_negativa)
ggsave("resultados_negativa/redes_rmse_sensitivity_plot.png", plot = redes_sensitivity_plot_rmse_negativa,
       width = 8, height = 5, dpi = 300)

# Se visualizan las 5 configuraciones con menor error, observando su variabilidad
redes_negativa_lollipop_plot <- ggplot(redes_best_configs_negativa, aes(x = etiqueta, y = mean_RMSE)) +
...
print(redes_negativa_lollipop_plot)
ggsave("resultados_negativa/redes_rmse_lollipot_plot.png", plot = redes_negativa_lollipop_plot,
       width = 8, height = 5, dpi = 300)
...
redes_mejor_config_negativa <- redes_patron_principal_procesada %>% 
  filter(learning_rate == best_lr & pairs_per_tick == best_pairs & confidence_threshold == best_confidence)

redes_mejor_config_stats_negativa <- redes_mejor_config_negativa %>% 
  group_by(tick) %>% 
  summarise(
...
redes_mejor_config_distance <- encuestas_preferencias_general %>%
  inner_join(redes_mejor_config_stats_negativa, by = "tick") %>%
  mutate(
...
redes_mejor_config_negativa_plot <- ggplot(redes_mejor_config_stats_negativa, aes(x = tick)) +
  ...
  theme_minimal() +
  labs(
    title = "Preference evolution best configuration in negative influence model with complex networks",
...
print(redes_mejor_config_negativa_plot)
ggsave("resultados_negativa/negativa_best_config_plot.png", plot = redes_mejor_config_negativa_plot,
       width = 8, height = 5, dpi = 300)
