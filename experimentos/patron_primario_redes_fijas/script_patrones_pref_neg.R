...
patron_pref_negativa <- read.csv("redes_preferenciales_data/main-pattern-negative-fixed-pref-table.csv", skip = 6)

# Se leen los datos empiricos de preferencia en el tiempo, para validar.
# Unicamente se considera la opcion A
encuestas_preferencias_general <- read.csv("redes_preferenciales_data/encuestas_preferencias_general.csv")
encuestas_preferencias_hombres <- read.csv("redes_preferenciales_data/encuestas_preferencias_A_hombres.csv") 
encuestas_preferencias_mujeres <- read.csv("redes_preferenciales_data/encuestas_preferencias_A_mujeres.csv")

# Cambio nombre de variables y selecciono unicamente las necesarias
patron_pref_negativa <- patron_pref_negativa %>% 
  rename(
    tick = X.step., 
...
patron_pref_negativa <- patron_pref_negativa %>% 
  select(run_number, tick, pairs_per_tick, learning_rate, confidence_threshold, pct_A, male_pct_A, female_pct_A)
...
patrones_por_configuracion <- patron_pref_negativa %>% 
  group_by(pairs_per_tick, learning_rate, confidence_threshold, tick) %>% 
  summarise(
    across(
...
rmse_por_corrida <- patron_pref_negativa %>%
  filter(tick %in% c(69, 166, 259)) %>%
...
best_confidence_threshold_general = best_config_general$confidence_threshold
...
best_confidence_threshold_hombres = best_config_hombres$confidence_threshold
...
best_confidence_threshold_mujeres = best_config_mujeres$confidence_threshold
...
simulation_best_config_hombres <- read.csv("redes_preferenciales_data/modelo_redes_preferenciales best-config-neg-hombres-pref-table",
                                           skip= 6) 
...
simulation_best_config_hombres_plot <- ggplot(simulation_best_config_hombres_largo, aes(x = tick, group = grupo)) +
  ...
  labs(
    title = "Intención de voto bajo influencia negativa en redes preferenciales,
    ...
    subtitle = paste0("pairs-per-tick = ", best_pairs_per_tick_hombres,
                      ...
                      ", confidence-threshold = ", best_confidence_threshold_hombres),
    x = "Ticks (días)",
    y = "Preferencia por A (%)",
    color = "Grupo",
    fill = "Grupo"
...
ggsave("figures_pref/neg_patron_hombres_pref.pdf", 
       plot = simulation_best_config_hombres_plot,
       width = 16, height = 10, units = "cm")

#### Simulación de mejor configuración general ####
simulation_best_config_general <- read.csv("redes_preferenciales_data/modelo_redes_preferenciales best-config-neg-general-pref-table",
                                           skip= 6) 
...
simulation_best_config_general_plot <- ggplot(simulation_best_config_general_largo, aes(x = tick, group = grupo)) +
  ...
  labs(
    ...
    subtitle = paste0("pairs-per-tick = ", best_pairs_per_tick_general,
                      ...
                      ", confidence-threshold = ", best_confidence_threshold_general),
    x = "Ticks (días)",
    y = "Preferencia por A (%)",
    color = "Grupo",
    fill = "Grupo"
...
# Exportación con las medidas calculadas para los márgenes del documento
ggsave("figures_pref/neg_patron_general_pref.pdf", 
...
simulation_best_config_mujeres <- read.csv("redes_preferenciales_data/modelo_redes_preferenciales best-config-neg-mujeres-pref-table",
                                           skip= 6) 
...
simulation_best_config_mujeres_plot <- ggplot(simulation_best_config_mujeres_largo, aes(x = tick, group = grupo)) +
  ...
  labs(
    ...
    subtitle = paste0("pairs-per-tick = ", best_pairs_per_tick_mujeres,
                      ...
                      ", confidence-threshold = ", best_confidence_threshold_mujeres),
    x = "Ticks (días)",
    y = "Preferencia por A (%)",
    color = "Grupo",
    fill = "Grupo"
...
# Exportación con las medidas calculadas para los márgenes del documento
ggsave("figures_pref/neg_patron_mujeres_pref.pdf", 
